 ...
 *   - X / Z fiscal reports
 *   - Cash IN / Cash OUT (custom do_nsoft_cash_op method - creates Odoo
 *     statement line, registers in nSoft, returns receipt text for printing)
 *
...
patch(Chrome.prototype, {
    ...
    async _nsoftPrintText(text) {
        ...
        try {
            ...
            this.notification.add(
                "Spausdintuvas nesukonfiguruotas (Settings -> POS -> ePos Printer).",
                { type: "warning", title: "Nera spausdintuvo" }
            );
            return false;
        } catch (e) {
    ...
    async _nsoftCashOp(direction, defaultTitle) {
        ...
        const reason = defaultTitle;
        try {
            const result = await this.orm.call("pos.session", "do_nsoft_cash_op",
                [[this.pos.session.id], direction, amount, reason]);
            console.log("[nSoft] do_nsoft_cash_op response:", result);
            if (result && result.success === false) {
                this.notification.add(result.message || "Klaida", {
                    type: "danger",
                    title: result.title || defaultTitle,
            ...
            this.notification.add(
                (result && result.message) || (amount.toFixed(2) + " EUR uzregistruota."),
                { type: "success", title: (result && result.title) || defaultTitle }
            );
        } catch (e) {
            this.notification.add((e.message || e).toString(),
