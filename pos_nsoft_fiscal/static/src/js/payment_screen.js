...
patch(Chrome.prototype, {
    ...
    /**
     * Send raw nVF text to nSoft Print Agent (lokalus tiltas tarp Odoo Cloud
     * ir spausdintuvo per port 9100 raw ESC/POS).
     *
     * Naudoja config.nsoft_print_agent_url + config.nsoft_printer_host +
     * config.nsoft_printer_port. Jei agent_url tuscias - krenta i Odoo
     * standartini ePos integracija (kuris veiks tik su TM-i serijos
     * spausdintuvais arba IoT Box).
     */
    async _nsoftPrintViaAgent(text) {
        const config = this.pos?.config;
        const agentUrl = (config?.nsoft_print_agent_url || '').trim();
        const host = (config?.nsoft_printer_host || '').trim();
        const port = parseInt(config?.nsoft_printer_port || 9100, 10);
        if (!agentUrl || !host) {
        ...
        try {
            ...
            return { ok: false, reason: data.message || `HTTP ${resp.status}` };
        } catch (e) {
            return { ok: false, reason: (e?.message || e).toString() };
        }
    },
    ...
    async _nsoftPrintText(text) {
        ...
        // 1. Pirmiausia bandom Print Agent (jei sukonfigūruotas)
        const agentRes = await this._nsoftPrintViaAgent(text);
        if (agentRes.ok) {
            console.log('[nSoft] Print agent OK:', agentRes.data);
            return true;
        }
        console.warn('[nSoft] Print agent failed:', agentRes.reason);

        // 2. Fallback - Odoo standartinis ePos printer (TM-i / IoT Box)
        ...
        try {
            ...
            // 3. Diagnostine klaida (NE fallback i window.print -
            //    VMI reikalauja, kad joks spausdinimo kelias neapeitu nVF)
            this.notification.add(
                'Spausdinimas nepavyko: ' + (agentRes.reason || e?.message || e),
                { type: 'danger', title: 'Spausdinimo klaida' });
            return false;
        }
        return false;
    },
