...
class ResConfigSettings(models.TransientModel):
    ...
    def _require_connection(self):
        ...
        if not self.pos_nsoft_enabled:
            raise UserError("Pirmiausia įjunkite nSoft fiskalizaciją.")
        if not self.pos_nsoft_api_url or not self.pos_nsoft_token:
            raise UserError("Užpildykite API URL ir Token laukus.")
    ...
    def action_test_nsoft_connection(self):
        ...
        mod = (data or {}).get('mod', '?')
        return self._notify("Ryšys veikia!",
    ...
    def action_nsoft_get_info(self):
        ...
        if not self.pos_nsoft_pos_id:
            raise UserError("Trūksta POS ID.")
        try:
        ...
        msg = (
            f"Fiskalinė diena Nr.: {content.get('fiscalDayNo', '-')}\n"
        ...
        return self._notify("Kasos būklė", msg)
    ...
    def action_nsoft_patch_settings(self):
        ...
        try:
            ...
            raise UserError(f"Nepavyko atnaujinti kasos nustatymų: {exc}")
        return self._notify("Kasos aparatas", "Nustatymai išsiųsti į i.EKA.")
    ...
    def action_nsoft_list_registers(self):
        ...
        return self._notify("Aktyvios kasos", ', '.join(content or []) or '(nėra)')
